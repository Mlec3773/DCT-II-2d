module dct2_32_1 #(parameter WIDTH =9)(input signed [WIDTH-1:0]X[0:31], 
output signed [WIDTH:0]Ye[0:15],
output signed [WIDTH+10:0]Yo[0:15]); 


//even-odd decomp
wire signed [WIDTH:0] E[0:15], O[0:15];

generate
genvar i;
	for (i = 0; i < 16; i = i + 1) begin: dcmploop
		decomp_0_1 #(.WIDTH(WIDTH))decomposicao(X[i], X[31-i], E[i], O[i]);
	end
endgenerate
...
wire signed [WIDTH+10:0] x4[0:15], x13[0:15], x22[0:15], x31[0:15],x38[0:15],x46[0:15],x54[0:15],x61[0:15],x67[0:15],x73[0:15],x78[0:15],x82[0:15],x85[0:15],x88[0:15],x90[0:15];

generate // SAU Setup
genvar j;
	for (j = 0; j < 16; j = j + 1) begin: oddSAUloop
		sau_16o_1 #(.WIDTH(WIDTH+1))sau_odd(O[j],x4[j], x13[j], x22[j], x31[j],x38[j],x46[j],x54[j],x61[j],x67[j],x73[j],x78[j],x82[j],x85[j],x88[j],x90[j]);
	end
endgenerate
