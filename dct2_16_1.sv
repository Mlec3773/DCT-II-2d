module dct2_16_1 #(parameter WIDTH =10)(input signed [WIDTH-1:0]X[0:15], 
output signed [WIDTH:0]Ye[0:7],
output signed [WIDTH+9:0]Yo[0:7]); 


//even-odd decomp
wire signed [WIDTH:0] E[0:7], O[0:7];

generate
genvar i;
	for (i = 0; i < 8; i = i + 1) begin: dcmploop
		decomp_1_1 #(.WIDTH(WIDTH))decomposicao(X[i], X[15-i], E[i], O[i]);
	end
endgenerate
...
wire signed [WIDTH+9:0] x9[0:7], x25[0:7], x43[0:7], x57[0:7], x70[0:7], x80[0:7], x87[0:7], x90[0:7];

generate // SAU Setup
genvar j;
	for (j = 0; j < 8; j = j + 1) begin: oddSAUloop
		sau_8o_1 #(.WIDTH(WIDTH+1))sau_odd(O[j],x9[j], x25[j], x43[j], x57[j], x70[j], x80[j], x87[j], x90[j]);
	end
endgenerate
